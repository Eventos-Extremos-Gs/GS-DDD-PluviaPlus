...
public class DispositivoRepository {
    ...
    public Map<String, Integer> gerarResumoPorLocalizacao() {
        String query = """
        SELECT l.cidade AS nome, COUNT(*) AS total
        FROM PP_DISPOSITIVO d
        JOIN PP_LOCALIZACAO l ON d.id_localizacao = l.id_localizacao
        GROUP BY l.cidade
    """;
        ...
        try (var connection = DatabaseConfig.getConnection();
            ...
            while (resultSet.next()) {
                ...
                int total = resultSet.getInt("total");
                resumo.put(cidade, total);
            }

        } catch (Exception e) {
            LOGGER.error("Erro ao gerar resumo por cidade", e);
        }

        return resumo;
    }

    public Map<String, List<String>> mapearModelosPorCidades() {
        String query = """
        SELECT d.modelo, l.cidade
        FROM PP_DISPOSITIVO d
        JOIN PP_LOCALIZACAO l ON d.id_localizacao = l.id_localizacao
    """;
        ...
        try (var connection = DatabaseConfig.getConnection();
            ...
            while (resultSet.next()) {
                ...
                mapa.computeIfAbsent(modelo, k -> new ArrayList<>()).add(cidade);
            }

        } catch (Exception e) {
            LOGGER.error("Erro ao mapear modelos por cidade", e);
        }
